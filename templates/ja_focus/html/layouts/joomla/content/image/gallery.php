...
if(is_array($gallery) && isset($gallery['src'])) {
	...
	if(count($gallery['src']) > 1) {
		if(!defined('TELINE_GALLERY_LIST_PLAY')) {
			...
			$path = JUri::root(true).'/templates/ja_focus/js/gallery/';
...
<?php if($context == 'icon'): ?>
	<?php if (isset($thumbnail) && !empty($thumbnail)) : ?>
		<div id="<?php echo $galleryId; ?>" class="btn-fullscreen ja-gallery-list">
			...
			<?php
				...
				<input type='hidden' value='<?php echo json_encode($images); ?>' />
		</div>
	<?php endif; ?>
<?php else: ?>
	<?php if (isset($thumbnail) && !empty($thumbnail)) : ?>
