...
$menuCount									= $helper->getRows('data.menu-name');
?>
<!-- HEADER -->
...
<?php endif; ?>
<header id ="uber-header-<?php echo $module->id; ?>" class="wrap uber-header header-8 dark-color">
	...
	<div class="row">
		...
		<!-- MAIN NAVIGATION -->
		<?php if($menuCount): ?>
		<nav class="navbar navbar-default t3-mainnav" role="navigation">
			<!-- Brand and toggle get grouped for better mobile display -->
			<div class="navbar-header">
				<?php if ($t3doc->getParam('navigation_collapse_enable', 1) && $t3doc->getParam('responsive', 1)) : ?>
					<?php $t3doc->addScript(T3_URL.'/js/nav-collapse.js'); ?>
			...
			<?php if ($t3doc->getParam('navigation_collapse_enable')) : ?>
				<div class="t3-navbar-collapse navbar-collapse collapse"></div>
			<?php endif ?>
			
			<div class="t3-navbar navbar-collapse collapse">
				<ul class="nav navbar-nav level0" role="tablist">
					<?php 
		 				for ($i=0; $i<$menuCount; $i++) : 
		 				
		 				$menuName = $helper->get('data.menu-name',$i);
						$menuLink = $helper->get('data.menu-link',$i);
						$moduleID = $helper->get('data.module-id',$i);
					?>
					<li <?php if($i==0): ?> class="active" <?php endif; ?> >
						<?php if($menuLink): ?>
							<a class="external" href="<?php echo $menuLink ?>"><?php echo $menuName; ?></a>
						<?php elseif($moduleID): ?>
							<a href="<?php echo $menuurl ?>#section-<?php echo $moduleID; ?>"><?php echo $menuName; ?></a>
						<?php else: ?>
							<a href="<?php echo $menuurl ?>#home"><?php echo $menuName; ?></a>
						<?php endif; ?>
					</li>
					<?php endfor ?>
				</ul>
		...
		<?php if ($headright): ?>
			<div class="site-navigation-right pull-right">
				...
				<?php if ($doc->countModules('right-menu')) : ?>
					<!-- RIGHT MENU -->
					<div class="right-menu" <?php if ($tplparams->get('addon_offcanvas_enable') && $fullWidth) : ?>style="margin-right: 40px;" <?php endif; ?>>
