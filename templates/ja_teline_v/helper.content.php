...
/**
 * BASED ON HELPER FOR MOD_ARTICLES_CATEGORY
 *
 * @package     Joomla.Site
 * @subpackage  mod_articles_category
 *
 * @since       1.6
 */
abstract class JATemplateHelperContent
{
	/**
	...
	/**
	 * @param JAContentTypeModelItems $articles
		...
		if(!empty($filter_preset)) {
			switch($filter_preset) {
				...
				case 'current_events':
					...
					$articles->metaFilter('content_type', 'event');
					$articles->metaFilter('start', $nowDate, '<=');
					//greater than current date or end date is not specify
					$articles->metaFilters('end', array('>= '.$db->quote($nowDate), '='.$db->quote('')));
					$articles->setMetaOrder('start', 'ASC');
					break;
				case 'past_events':
					//Upcoming Events
