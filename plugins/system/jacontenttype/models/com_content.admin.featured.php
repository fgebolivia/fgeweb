	...
	protected function getListQuery()
	{
		...
		if ($content_type)
		{
			if($content_type == 'article') {
				$query->leftJoin('#__content_meta AS jactt ON (jactt.content_id = a.id AND jactt.meta_key = '.$db->quote('content_type').')');
				$query->where('jactt.'.$db->quoteName('meta_value').' IS NULL');
			} else {
				$query->innerJoin('#__content_meta AS jactt ON (jactt.content_id = a.id AND jactt.meta_key = '.$db->quote('content_type').')');
				$query->where('jactt.'.$db->quoteName('meta_value').'='.$db->quote($content_type));
			}
		}
