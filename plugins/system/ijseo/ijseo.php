...
class plgSystemIjseo extends JPlugin{    
			...
			case "com_k2" : {
				...
				if ($layout == "item" || $layout == "category" || $view == "item" || $view == "category" || $view == "itemlist") {
					if ($layout == "category" || $view == "itemlist") {
						$sql = "SELECT c.`params`, m.titletag FROM #__k2_categories c LEFT JOIN #__ijseo_metags m on m.id=c.id and m.`mtype`='k2-cat' WHERE c.`id`=".intval($id);
						$db->setQuery($sql);
						$obj = $db->loadObject();
					...
					elseif ($layout == "item" || $view == "item") {
						$sql = "SELECT c.`metadesc`, c.`metakey`, m.`titletag` FROM #__k2_items c LEFT JOIN #__ijseo_metags m on m.id=c.id and m.`mtype`='k2-item' WHERE c.`id`=".intval($id);
						$db->setQuery($sql);
						$obj = $db->loadObject();
	...
	function onContentPrepare(){
		...
		if($params->ijseo_gposition){			
			...
			/*if($current_date_for_ping <= (strtotime($last_ping_date)+30)){
				...
				$sql = " select * from #__ijseo_keys as a where ".$check_grank."<=(SELECT DATEDIFF('".$date."',a.checkdate)) OR a.checkdate='0000-00-00 00:00:00' order by checkdate asc limit 0,10 ";
				$db->setQuery($sql);
				if(!$db->query()){
					echo $db->getErrorMsg();
				}	
	...
	function updateRank($oldrank, $newrank, $currentDate, $key, $ijseo_keysource){
		...
		$sql = "update #__ijseo_keys set rank = ".$newrank." , rchange =  ".$change.", mode = ".$mode." , checkdate = '".$currentDate."' where title = '".$db->escape($key)."' ";
		$db->setQuery($sql);
		if(!$db->query()){
	...
	function onAfterRender(){
		...
		if($option == "com_community" || $option == "com_zoo" || $option == "com_kunena" || $option == "com_k2" || $option == "com_easyblog" || $option == "com_virtuemart" || $option == "com_content"){
			$doc = JFactory::getDocument();
			$style = '.iseo-dashed{
						border-bottom:1px dashed;
						text-decoration:none !important;
						...
						case 1:
							if($val['articleId'] != $articolId){
								...
								if($val["other_phrases"]==1){
									$pattern[] = '\'(?!((<.*?)|(<a.*?)|(<script.*?)|(<style.*?)))('.$temp_replace.')(?!(([^<>]*?)>)|([^>]*?</a>)|([^>]*?</span></a>)|([^>]*?</script>)|([^>]*?</style>))\'s'.$sensitive."u";
									$replace[] ='<a href="'.$url.'" title="'.$val['title'].'" target="'.$target.'" >'.$val['name'].'</a>';
								}
								else{
									$pattern[] = '\'(?!((<.*?)|(<a.*?)|(<script.*?)|(<style.*?)))(\b '.$temp_replace.' \b)(?!(([^<>]*?)>)|([^>]*?</a>)|([^>]*?</span></a>)|([^>]*?</script>)|([^>]*?</style>))\'s'.$sensitive."u";
									$pattern[] = '\'(?!((<.*?)|(<a.*?)|(<script.*?)|(<style.*?)))(\b'.$temp_replace.'\b)(?!(([^<>]*?)>)|([^>]*?</a>)|([^>]*?</span></a>)|([^>]*?</script>)|([^>]*?</style>))\'s'.$sensitive."u";
	...
	function titleplg(){				
		...
		if(!empty($className1) && $replaceH1!='noreplace'){							
			$this->replaceClass($className1, $replaceH1);			
		}
		if(!empty($className2) && $replaceH2!='noreplace'){			
			$this->replaceClass($className2, $replaceH2);
		}
		if(!empty($className3) && $replaceH3!='noreplace'){
			$this->replaceClass($className3, $replaceH3);
		}
		if(!empty($className4) && $replaceH4!='noreplace'){
			$this->replaceClass($className4, $replaceH4);
		}
		if(!empty($className5) && $replaceH5!='noreplace'){
			$this->replaceClass($className5, $replaceH5);
		}
		if(!empty($className6) && $replaceH6!='noreplace'){
			$this->replaceClass($className6, $replaceH6);
		}
	}
	
	function replaceClass($className, $replaceH){		
