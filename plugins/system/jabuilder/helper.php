...
class JUBHelper {
	...
	public function jubMode() {
		return $this->app->input->request->getCmd('jub');
	}
	...
	public function isEditable() {
		return $this->isRootUser() || defined('JUB_SITE_KEY');
	}
    ...
    private function getDataInput () {
		...
		$data = $input->request->getRaw('data');
	...
	public function doLoadConfig() {
		...
		$config['rev']['page'] = $db->setQuery ('Select max(rev) from #__jabuilder_revisions where itemid=' . (int)$page->id)->loadResult();

		$layout = $this->getKey('layout', true);
		$config['rev']['layout'] = $db->setQuery ('Select max(rev) from #__jabuilder_revisions where itemid=' . (int)$layout->id)->loadResult();
...
*/
	...
	public function doSaveCss() {
		...
		$data = $this->getDataInput();
		$published = (boolean)$input->request->get('published');
		$site = defined('JUB_SITE_KEY') ? JUB_SITE_KEY : 'default';
	...
	protected function addJUBAssetsFromSettings($fromsettings) {
		...
		if (isset($fromsettings['webfonts']) && count($fromsettings['webfonts'])) {
			$webfonts = '';
			$webfontCls = '';
			foreach ($fromsettings['webfonts'] as $name => $font) {
				$webfonts .= str_replace(' ', '+', $name);
				if (is_array($font['styles']) && count($font['styles']) > 1) {
					$webfonts .= ':' . implode(',', $font['styles']);
				}
				$webfonts .= '|';
