...
class iJoomla_SeoControllerAbout extends iJoomla_SeoController{
	...
	function calculateVirtuemartStats(){
		$database	= JFactory::getDBO();
		$query		= $database->getQuery(true);
		$value = JRequest::getVar("value", "");
		$virtuemart_option = JRequest::getVar("virtuemart_id", "");
		
		$sql = "SHOW TABLES";
		$database->setQuery($sql);
		$tables = $database->loadColumn();
		$config = JFactory::getConfig();
		if (!in_array($config->get( 'dbprefix' ) . "virtuemart_products", $tables)) {
			echo "";
			die();
		}
	...
	function getCategoriesListing(){
		...
		switch ($value){
			case "title":
				...
				$href = 'index.php?option=com_ijoomla_seo&controller=virtuemart&virtuemart=1&atype=1';
				break;
			case "keys":
				$where.= " and c.`metakey` like '' ";
				$href = 'index.php?option=com_ijoomla_seo&controller=virtuemart&virtuemart=1&atype=2';
				break;
			case "desc":
				$where.= " and c.`metadesc` like ''";
				$href = 'index.php?option=com_ijoomla_seo&controller=virtuemart&virtuemart=1&atype=3';
				break;
			default:
				break;
		}
		...
		$query->from('#__virtuemart_categories_en_gb c');
		$query->where($where);
	...
	function getProductsListing(){
		...
		switch ($value){
			case "title":
				...
				$href = 'index.php?option=com_ijoomla_seo&controller=virtuemart&virtuemart=2&atype=1';
				break;
			case "keys":
				$where.= " and p.`metakey` like '' ";
				$href = 'index.php?option=com_ijoomla_seo&controller=virtuemart&virtuemart=2&atype=2';
				break;
			case "desc":
				$where.= " and p.`metadesc` like ''";
				$href = 'index.php?option=com_ijoomla_seo&controller=virtuemart&virtuemart=2&atype=3';
				break;
			default:
				break;
		}
		...
		$query->from('#__virtuemart_products_en_gb p');
		$query->where($where);
