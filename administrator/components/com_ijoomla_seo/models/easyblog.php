...
class iJoomla_SeoModelEasyblog extends JModelLegacy {
	...
	function getListQuery() {
		...
		if($filter_search!=""){ 
			$where.=" AND (k.`title` LIKE '%".addslashes($filter_search)."%') ";
		}
	...
	function getIntroFullText($id){
		...
		$query = $db->getQuery(true);
		$query->clear();		
		$query->select('`intro`, `excerpt`, `content`');
	...
	function genMetadesc(){
		...
		foreach($ids as $key=>$id){
			...
			if(trim($intr_full["0"]["intro"]) == ""){
				$intr_full["0"]["intro"] = $intr_full["0"]["excerpt"];
			}

			if(trim($intr_full["0"]["intro"]) == ""){
				$intr_full["0"]["intro"] = $intr_full["0"]["content"];
			}
